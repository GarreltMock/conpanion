    ...
    return (
        <View>
            <View>
                ...
                {!readOnly && (
                    <View>
                        <Pressable onPress={handleEditNote} testID="edit-button" accessibilityRole="button">
                            <Text>Edit</Text>
                        </Pressable>

                        <Pressable onPress={handleDeleteNote} testID="delete-button" accessibilityRole="button">
                            <Text>Delete</Text>
                        </Pressable>
                    </View>
                )}
            ...
            {note.audioRecordings.length > 0 && (
                <View>
                    {note.audioRecordings.map((_, index) => (
                        <Pressable key={`audio-${index}`} testID={`audio-player-${index}`} accessibilityRole="button">
                            <View />
...
describe("NoteItem Component", () => {
    ...
    test("renders note content correctly", () => {
        const { getByText } = render(<NoteItem note={mockNote} onDelete={mockDeleteFn} />);
    ...
    test("handles edit button press", () => {
        const { getByTestId } = render(<NoteItem note={mockNote} onDelete={mockDeleteFn} />);
    ...
    test("does not show edit/delete buttons in readOnly mode", () => {
        const { queryByTestId } = render(<NoteItem note={mockNote} readOnly={true} />);
