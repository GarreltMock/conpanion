...
export const ExportOptionsForm: React.FC<ExportOptionsFormProps> = ({
  ...
  useEffect(() => {
    ...
    loadSavedOptions();
  }, [conferenceId, conference, conferenceTalks, getExportOptions]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (options.format === 'pdf') {
        exportPath = await exportToPDF(conferenceId, options);
      } else {
        exportPath = await exportToMarkdown(conferenceId, options);
      }
        ...
        <ThemedView style={styles.formSection}>
          ...
          <View style={styles.formatOptions}>
            <TouchableOpacity
              ...
              onPress={() => setOptions(prev => ({ ...prev, format: 'pdf' }))}
            >
              <Ionicons 
              ...
              <ThemedText style={[
                styles.formatLabel,
                options.format === 'pdf' && { color: tintColor, fontWeight: 'bold' }
              ]}>
                PDF
              </ThemedText>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.formatOption,
                options.format === 'markdown' && styles.selectedFormat,
                { borderColor: tintColor }
              ]}
              onPress={() => setOptions(prev => ({ ...prev, format: 'markdown' }))}
            >
              <Ionicons 
                name="code-outline" 
                size={24} 
                color={options.format === 'markdown' ? tintColor : textColor} 
              />
              <ThemedText style={[
                styles.formatLabel,
                options.format === 'markdown' && { color: tintColor, fontWeight: 'bold' }
              ]}>
                Markdown
              </ThemedText>
            </TouchableOpacity>
          ...
          <View style={styles.optionRow}>
            ...
            <Switch
              ...
              onValueChange={(value) => setOptions(prev => ({ ...prev, includeImages: value }))}
              trackColor={{ false: placeholderColor, true: tintColor }}
            />
          </View>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity
              style={[styles.button, styles.exportButton, { backgroundColor: tintColor, opacity: isSubmitting ? 0.7 : 1 }]}
